 ...
 */
  getTreeInfo():Promise<any[]>{
    ...
    const url = urlArr[0]+"//"+urlArr[2]+"/indextree/jsonmapping/"+urlArr[urlArr.length-1];
    //APIからtree情報を取得する
...
　setNodeInfo(treeModel:TreeModel):Promise<void>{
   ...
   */
  setCheckedNode(url:string,nodeList:any):Promise<void>{
    return this.http
    .put(url,nodeList,this.options)
    .toPromise()
    .then(response => response.json() as any)
    .catch(this.handleError);
  }
